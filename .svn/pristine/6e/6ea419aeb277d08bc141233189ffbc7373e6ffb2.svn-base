...
", id, mineCode, simpleName, city, fullName, type,
    ...
    maCode, mineGroup, coordinates, remark);
        ...
        public void MineConfig_Delete(string condition)
        {
            ...
            string sql3 = string.Format("delete from aqss  where minecode in  ( select minecode from mineconfig where id in ({0}))", idString);
            string sql2 = string.Format("delete from systemconfig where minecode in  ({0})", idString);
            string sql =
string.Format(
"Delete From MineConfig Where Id In ({0})", idString);


            bool result1 = _mineCfgModel.Exec(sql3);
            bool result2= _mineCfgModel.Exec(sql2);
            bool result3 = _mineCfgModel.Exec(sql);
